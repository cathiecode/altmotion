...
pub struct SequenceRenderer<'a, T> where T: Renderer + 'a {
    clip_renderer_registory: &'a mut ClipRegistory<T::Image>,
    sequence: &'a Sequence,
...
impl<'a, T> SequenceRenderer<'a, T> where T: Renderer {
    pub fn new<V>(clip_renderer_registory: &'a mut ClipRegistory<V::Image>, sequence: &'a Sequence) -> SequenceRenderer<'a, V> where V: Renderer {
        SequenceRenderer {
        ...
        for layer in self.sequence.layers.iter() {
            ...
            if let Some(current_clip) = current_clip_or_none {
                ...
                if let Some(renderer_factory) = renderer_factory_or_none {
                    let mut renderer = renderer_factory.new();
                    let objects = renderer.render(&ClipRenderContext {});
